...
async def currency_chosen(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    reply_markup = InlineKeyboardMarkup(keyboard)
    await query.edit_message_text(f"Вы выбрали валюту {currency}. Выберите интервал (не используется для уведомлений, только для справки):", reply_markup=reply_markup)

# --- Выбор интервала и запрос порогов ---
...
# --- Обработка пользовательского ввода порогов ---
...
async def status(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not (coin and currency and interval and last_price):
        await update.message.reply_text("Настройка не завершена. Пожалуйста, выберите монету, валюту и интервал через /start.")
        return
    ...
    await update.message.reply_text(
        ...
        f"Монета: {coin}\n"
        f"Валюта: {currency}\n"
        f"Интервал (не используется): {interval}\n"
